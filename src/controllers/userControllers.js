...
const createUsers = async function (req, res) {
    try {
        ...
        if (!isValidName(name))
            return res.status(400).send({ status: false, message: "Name should be in alphabets" })
        data.name = name;

        // ----->> phone validation <<-----
        if (!isPresent(phone)) {
            return res.status(400).send({ status: false, message: "Please provide the phone no." })
        }
        ...
        if (phoneNoCheck) {
            return res.status(400).send({ status: false, message: "This phone number is already registerd" })
        }
        data.phone = phone;
